...
class DataGenerator(keras.utils.all_utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            file_path = os.path.join(base_path,item["mw_file_directory"])
...
class AutoencoderLossLayer(Layer):
    ...
    def call(self, inputs):
        ...
        self.add_loss(loss, inputs=inputs)#排除这里
        print("loss2 shape after:", loss.shape)
    ...
    def compute_output_shape(self, input_shape):
        ...
        print("after compute_output_shape:", tuple(K.int_shape(self.results)))
...
class GradientBoostingDeicisionTreeLayer(Layer):
    ...
    def neural_decision_trees(self, decision_p_e, n_depth, n_leaf, n_batch):
        ...
        for i, flat_decision_p in enumerate(flat_decision_p_e):
            mu = tf.gather(flat_decision_p, tf.add(batch_0_indices, batch_complement_indices))#排除这里
            #mu = tf.gather(flat_decision_p, batch_0_indices)
            mu_e.append(mu)
        ...
        for d in range(1, n_depth + 1):
            ...
            tile_indices = tf.reshape(tf.tile(tf.expand_dims(indices, 1),
                                              [1, 2 ** (n_depth - d + 1)]), [1, -1])
            batch_indices = tf.add(batch_0_indices, tf.tile(tile_indices, [n_batch, 1]))#排除这里
            #batch_indices = batch_0_indices
            ...
            for mu, flat_decision_p in zip(mu_e, flat_decision_p_e):
                mu = tf.multiply(mu, tf.gather(flat_decision_p, tf.add(batch_indices, batch_complement_indices)))#排除这里
    ...
    def probability_y_x(self, mu_e, leaf_p_e, b_l_e, y_true, batch_size):
        ...
        for i in range(self.n_tree):
            ...
            print(tf.reduce_sum(tf.multiply(mu, tf.tile(tf.expand_dims(leaf_p, 0), [batch_size, 1])),keepdims=True, axis=1).shape, b_l,f_i.shape,predict.shape)
            ...
            # all_predict.append(tf.tile(predict, [batch_size, ]))# TODO 报错换了个地方
            # loss += 0
...
class Proposed_Model():
    ...
    def get_model(self):
        ...
        y = AutoencoderLossLayer()([x_input, decoded, y])#去掉这层仍然报错
    ...
    def predict(self, model_name, x_test, y_test, batch_size=32):
        model = load_model(model_name,
                           custom_objects={'GradientBoostingDeicisionTreeLayer': GradientBoostingDeicisionTreeLayer,
                                           'AutoencoderLossLayer': AutoencoderLossLayer,#排除这里
...
proposed_model_name = 'proposed_model_no_loss'
prefix = "../saved_models/"
